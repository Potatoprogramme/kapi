...
class ProductsController < ApplicationController
  ...
  def load_form_data(product_id = nil)
    ...
    @ingredients = Ingredient.left_joins(:ingredient_costing).left_joins(:material)
                             ...
                             .select('ingredients.*, materials.*, ingredient_costings.quantity, ingredient_costings.ingredient_total_cost')
  end
  ...
  def insert_ingredient(method = nil)
    ...
    ingredient_params.each do |material|
      ...
      insert_ingredient_costing(new.id, material['quantity'], material['costPerUnit'])
    end
  end
